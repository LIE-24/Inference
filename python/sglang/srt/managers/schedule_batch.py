...
class Req:
    ...
    def __init__(
        ...
        self.output_ids = []
        # Ready if it is a new request or recv output from previous rank, otherwise it is not ready
        self.ready = True
        # fill_ids = origin_input_ids + output_ids. Updated if chunked.
        self.fill_ids = None
        self.session_id = session_id
        self.input_embeds = input_embeds
        self.pp_hidden_states = None
        self.pp_residual = None
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def select_batch(
        ...
        ret.token_to_kv_pool_allocator = self.token_to_kv_pool_allocator
        ret.req_to_token_pool = self.req_to_token_pool
        ret.tree_cache = self.tree_cache

        if self.model_config.is_encoder_decoder:
            ret.encoder_lens = self.encoder_lens[keep_indices_device]
            ret.encoder_lens_cpu = [self.encoder_lens_cpu[i] for i in keep_indices]

        ret.reqs = [self.reqs[i] for i in keep_indices]
        if self.multimodal_inputs is not None:
            ret.multimodal_inputs = [self.multimodal_inputs[i] for i in keep_indices]
        ret.req_pool_indices = self.req_pool_indices[keep_indices_device]
        ret.seq_lens = self.seq_lens[keep_indices_device]
        
        if self.out_cache_loc is not None:
            ret.out_cache_loc = self.out_cache_loc[keep_indices_device]
        ret.seq_lens_sum = ret.seq_lens.sum().item()
        
        # input ids is the last token of the output ids
        ret.output_ids = self.output_ids[keep_indices_device]
        # ret.output_ids = torch.tensor([req.output_ids[-1] for req in ret.reqs], dtype=torch.int64, device=self.device)

        ret.return_logprob = any(req.return_logprob for req in self.reqs)
        if ret.return_logprob:
            ret.top_logprobs_nums = [self.top_logprobs_nums[i] for i in keep_indices]
            ret.token_ids_logprobs = [self.token_ids_logprobs[i] for i in keep_indices]
        else:
            ret.top_logprobs_nums = None
            ret.token_ids_logprobs = None

        ret.has_stream = any(req.stream for req in self.reqs)
        ret.has_grammar = any(req.grammar for req in self.reqs)
        
        # TODO
        # ret.sampling_info.filter_batch(keep_indices, keep_indices_device)
        ret.sampling_info = SamplingBatchInfo.from_schedule_batch(ret, self.model_config.vocab_size)
        # if self.spec_info:
            # self.spec_info.filter_batch(keep_indices_device)
    ...
    def merge_batch(self, other: "ScheduleBatch"):
        ...
        if self.output_ids is not None:
            self.output_ids = torch.cat([self.output_ids, other.output_ids if other.output_ids is not None else torch.zeros(len(other.reqs), dtype=torch.int64, device=self.device)])
