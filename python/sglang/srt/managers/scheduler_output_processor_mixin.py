...
class SchedulerOutputProcessorMixin:
        ...
        if self.is_generation:
            ...
            for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
                ...
                if req.is_chunked <= 0:
                    ...
                    req.check_finished()
                    logger.debug(f"req {req.rid} next_token_id: {next_token_id}, check finished: {req.finished()}")
                    if req.finished():
    ...
    def process_batch_result_decode(
        ...
        print(f"process_batch_result_decode")
        ...
        for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
            ...
            req.check_finished()
            if req.finished():
                logger.info(f"req {req.rid} finished")
                self.tree_cache.cache_finished_req(req)
                req.time_stats.completion_time = time.time()
